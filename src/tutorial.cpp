...
void Tutorial::resetScene(Direction direction){
    ...
    level.spawn={{stage==4?300:430,(static_cast<int>(direction)%2)?402:362},direction};
    scene=Game(level);movement={};aim.reset();attempted=accepted=false;initialLock=-1;heldFrames=0;
    if(stage==4){scene.shoot({0,{260,389}});scene.shoot({1,{720,389}});}
}
void Tutorial::reset(int step){stage=std::clamp(step,0,4);frame=phase=0;resetScene();}
void Tutorial::update(){
    const int cycle=stage==0?240:stage==1?240:stage==2?360:stage==3?480:480;
