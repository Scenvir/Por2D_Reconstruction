...
$core = @('src/levels.cpp', 'src/portal.cpp', 'src/physics.cpp', 'src/game.cpp') |
...
$flags += '-static'
& $Compiler @flags @core (Join-Path $projectRoot 'src/render.cpp') (Join-Path $projectRoot 'src/main.cpp') '-lgdi32' '-luser32' '-mwindows' '-o' (Join-Path $buildDir 'por2.exe')
if ($LASTEXITCODE -ne 0) { throw 'Game build failed.' }
Write-Host "Built $buildDir\por2.exe"
if ($Test) {
